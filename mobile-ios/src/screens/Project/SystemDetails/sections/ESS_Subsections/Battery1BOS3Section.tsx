// src/screens/Project/SystemDetails/sections/ESS_Subsections/Battery1BOS3Section.tsx
...
interface Battery1BOS3SectionProps {
  values: {
    isNew: boolean;
...
const Battery1BOS3Section: React.FC<Battery1BOS3SectionProps> = ({
  values,
  onChange,
  errors,
  label = "Battery Type 1 BOS 3",
  ...
  useEffect(() => {
    const loadPreferred = async () => {
      ...
      try {
        ...
        console.log(`[Battery1BOS3Section] Loaded ${preferred.length} preferred equipment for type: "${values.equipmentType}" → "${equipmentType}"`);
        ...
        console.error("[Battery1BOS3Section] Error loading preferred equipment:", error);
      } finally {
  ...
  useEffect(() => {
    ...
    if (availableModels.length === 1) {
      console.log(`[Battery1BOS3Section] Auto-selecting single available model: ${availableModels[0]} (was: ${values.model})`);
      onChange("model", availableModels[0]);
    } else if (availableModels.length > 1) {
      console.log(`[Battery1BOS3Section] Multiple models available (${availableModels.length}), user must select`);
    }
...
export default Battery1BOS3Section;
