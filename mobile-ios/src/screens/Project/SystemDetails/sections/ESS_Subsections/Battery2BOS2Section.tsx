// src/screens/Project/SystemDetails/sections/ESS_Subsections/Battery2BOS2Section.tsx
...
interface Battery2BOS2SectionProps {
  values: {
    isNew: boolean;
...
const Battery2BOS2Section: React.FC<Battery2BOS2SectionProps> = ({
  values,
  onChange,
  errors,
  label = "Battery Type 2 BOS 2",
  ...
  useEffect(() => {
    const loadPreferred = async () => {
      ...
      try {
        ...
        console.log(`[Battery2BOS2Section] Loaded ${preferred.length} preferred equipment for type: "${values.equipmentType}" → "${equipmentType}"`);
        ...
        console.error("[Battery2BOS2Section] Error loading preferred equipment:", error);
      } finally {
  ...
  useEffect(() => {
    ...
    if (availableModels.length === 1) {
      console.log(`[Battery2BOS2Section] Auto-selecting single available model: ${availableModels[0]} (was: ${values.model})`);
      onChange("model", availableModels[0]);
    } else if (availableModels.length > 1) {
      console.log(`[Battery2BOS2Section] Multiple models available (${availableModels.length}), user must select`);
    }
...
export default Battery2BOS2Section;
