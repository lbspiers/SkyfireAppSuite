// src/screens/Project/SystemDetails/sections/ESS_Subsections/Battery2BOS3Section.tsx
...
interface Battery2BOS3SectionProps {
  values: {
    isNew: boolean;
...
const Battery2BOS3Section: React.FC<Battery2BOS3SectionProps> = ({
  values,
  onChange,
  errors,
  label = "Battery Type 2 BOS 3",
  ...
  useEffect(() => {
    const loadPreferred = async () => {
      ...
      try {
        ...
        console.log(`[Battery2BOS3Section] Loaded ${preferred.length} preferred equipment for type: "${values.equipmentType}" → "${equipmentType}"`);
        ...
        console.error("[Battery2BOS3Section] Error loading preferred equipment:", error);
      } finally {
  ...
  useEffect(() => {
    ...
    if (availableModels.length === 1) {
      console.log(`[Battery2BOS3Section] Auto-selecting single available model: ${availableModels[0]} (was: ${values.model})`);
      onChange("model", availableModels[0]);
    } else if (availableModels.length > 1) {
      console.log(`[Battery2BOS3Section] Multiple models available (${availableModels.length}), user must select`);
    }
...
export default Battery2BOS3Section;
