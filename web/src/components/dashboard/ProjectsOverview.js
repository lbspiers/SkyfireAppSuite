...
const ProjectsOverview = ({ projects = [], loading = false, selectedStatus = 'all', onProjectsUpdate, onViewChatter, onNewProject, isNewProjectOpen = false }) => {
  ...
  const [showSkyfireProjects, setShowSkyfireProjects] = useState(() => {
    // Default to FALSE (hide Skyfire projects)
    const stored = localStorage.getItem('showSkyfireProjects');
    return stored === 'true'; // Only true if explicitly set
  });
  ...
  // Skyfire Solar Design company ID
  const SKYFIRE_COMPANY_ID = 1;
  ...
  // Persist Skyfire filter preference
  useEffect(() => {
    localStorage.setItem('showSkyfireProjects', showSkyfireProjects.toString());
  }, [showSkyfireProjects]);

  // Detect newly added projects and highlight them
  useEffect(() => {
    const prevProjectIds = new Set(projectsData.map(p => p.id || p.uuid));
    const newIds = projects
      .filter(p => !prevProjectIds.has(p.id || p.uuid))
      .map(p => p.id || p.uuid);

    if (newIds.length > 0) {
      // Add new project IDs to highlight set
  ...
  const filteredProjects = useMemo(() => {
    ...
    // Filter out Skyfire projects unless toggle is ON (super admin only)
    if (isSuperAdmin && !showSkyfireProjects) {
      filtered = filtered.filter(p => p.company?.id !== SKYFIRE_COMPANY_ID);
    }
    ...
    return filtered;
  }, [projectsData, searchQuery, selectedStatus, showRecentOnly, showSkyfireProjects, isSuperAdmin, SKYFIRE_COMPANY_ID]);
  ...
  // Row class rules for highlighting selected and new projects
  const getRowClass = (params) => {
    const projectId = params.data?.id || params.data?.uuid;
    const isNew = newProjectIds.has(projectId);
    const isSelected = params.node.isSelected();

    if (isNew && isSelected) {
      return 'ag-row-selected-custom newProjectHighlight';
    } else if (isNew) {
  ...
  return (
    <div className={styles.projectsOverviewContainer}>
      ...
      <div className={styles.filters}>
        ...
        {isSuperAdmin && (
          <button
            className={`${styles.skyfireToggle} ${showSkyfireProjects ? styles.skyfireToggleActive : ''}`}
            onClick={() => setShowSkyfireProjects(prev => !prev)}
            title={showSkyfireProjects ? 'Hide Skyfire test projects' : 'Show Skyfire test projects'}
          >
            SF
          </button>
        )}
