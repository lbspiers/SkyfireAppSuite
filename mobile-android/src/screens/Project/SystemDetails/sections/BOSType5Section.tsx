...
const BOSType5Section: React.FC<BOSType5SectionProps> = ({
  values,
  onChange,
  errors,
  ...
  useEffect(() => {
    const loadPreferred = async () => {
      ...
      try {
        ...
        console.log(`[BOSType5Section] Loaded ${preferred.length} preferred equipment for type: "${values.equipmentType}" → "${equipmentType}"`);
        ...
        console.error("[BOSType5Section] Error loading preferred equipment:", error);
      } finally {
  ...
  useEffect(() => {
    ...
    if (availableModels.length === 1) {
      console.log(`[BOSType5Section] Auto-selecting single available model: ${availableModels[0]} (was: ${values.model})`);
      onChange("model", availableModels[0]);
    } else if (availableModels.length > 1) {
      console.log(`[BOSType5Section] Multiple models available (${availableModels.length}), user must select`);
    }
  ...
  return (
    ...
    >
      ...
      {/* Add BOS Type 6 Button */}
      {showAddBOSType6Button && onAddBOSType6 && (
        <View style={styles.addBOSButtonContainer}>
          <SystemButton
            label="Add Pre-Combine BOS Equipment"
            onPress={onAddBOSType6}
...
export default BOSType5Section;
