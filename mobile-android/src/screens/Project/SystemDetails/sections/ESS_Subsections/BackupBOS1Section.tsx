// src/screens/Project/SystemDetails/sections/ESS_Subsections/BackupBOS1Section.tsx
...
interface BackupBOS1SectionProps {
  values: {
    isNew: boolean;
...
const BackupBOS1Section: React.FC<BackupBOS1SectionProps> = ({
  values,
  onChange,
  errors,
  label = "Backup Load Subpanel BOS 1",
  ...
  useEffect(() => {
    const loadPreferred = async () => {
      ...
      try {
        ...
        console.log(`[BackupBOS1Section] Loaded ${preferred.length} preferred equipment for type: "${values.equipmentType}" → "${equipmentType}"`);
        ...
        console.error("[BackupBOS1Section] Error loading preferred equipment:", error);
      } finally {
  ...
  useEffect(() => {
    ...
    if (availableModels.length === 1) {
      console.log(`[BackupBOS1Section] Auto-selecting single available model: ${availableModels[0]} (was: ${values.model})`);
      onChange("model", availableModels[0]);
    } else if (availableModels.length > 1) {
      console.log(`[BackupBOS1Section] Multiple models available (${availableModels.length}), user must select`);
    }
...
export default BackupBOS1Section;
