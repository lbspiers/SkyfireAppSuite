// src/screens/Project/SystemDetails/sections/ESS_Subsections/BackupBOS3Section.tsx
...
interface BackupBOS3SectionProps {
  values: {
    isNew: boolean;
...
const BackupBOS3Section: React.FC<BackupBOS3SectionProps> = ({
  values,
  onChange,
  errors,
  label = "Backup Load Subpanel BOS 3",
  ...
  useEffect(() => {
    const loadPreferred = async () => {
      ...
      try {
        ...
        console.log(`[BackupBOS3Section] Loaded ${preferred.length} preferred equipment for type: "${values.equipmentType}" → "${equipmentType}"`);
        ...
        console.error("[BackupBOS3Section] Error loading preferred equipment:", error);
      } finally {
  ...
  useEffect(() => {
    ...
    if (availableModels.length === 1) {
      console.log(`[BackupBOS3Section] Auto-selecting single available model: ${availableModels[0]} (was: ${values.model})`);
      onChange("model", availableModels[0]);
    } else if (availableModels.length > 1) {
      console.log(`[BackupBOS3Section] Multiple models available (${availableModels.length}), user must select`);
    }
...
export default BackupBOS3Section;
