// src/screens/Project/SystemDetails/sections/ESS_Subsections/BackupBOS2Section.tsx
...
interface BackupBOS2SectionProps {
  values: {
    isNew: boolean;
...
const BackupBOS2Section: React.FC<BackupBOS2SectionProps> = ({
  values,
  onChange,
  errors,
  label = "Backup Load Subpanel BOS 2",
  maxContinuousOutputAmps = null,
  loadingMaxOutput = false,
  onRemove,
  showAddBOS3Button = false,
  onAddBOS3,
  utilityAbbrev,
  sizingLabel,
  sizingCalculation,
  ...
  useEffect(() => {
    const loadPreferred = async () => {
      ...
      try {
        ...
        console.log(`[BackupBOS2Section] Loaded ${preferred.length} preferred equipment for type: "${values.equipmentType}" → "${equipmentType}"`);
        ...
        console.error("[BackupBOS2Section] Error loading preferred equipment:", error);
      } finally {
  ...
  useEffect(() => {
    ...
    if (availableModels.length === 1) {
      console.log(`[BackupBOS2Section] Auto-selecting single available model: ${availableModels[0]} (was: ${values.model})`);
      onChange("model", availableModels[0]);
    } else if (availableModels.length > 1) {
      console.log(`[BackupBOS2Section] Multiple models available (${availableModels.length}), user must select`);
    }
    ...
    >
      ...
      <View style={styles.noteContainer}>
        <Text style={styles.noteText}>
          Note: BOS equipment is entered in order from the backup load subpanel back to the inverter/combiner panel.
        </Text>
      </View>
...
export default BackupBOS2Section;
