// src/screens/Project/SystemDetails/sections/BOSType2Section.tsx
...
interface BOSType2SectionProps {
  values: {
    isNew: boolean;
...
const BOSType2Section: React.FC<BOSType2SectionProps> = ({
  values,
  onChange,
  errors,
  ...
  useEffect(() => {
    const loadPreferred = async () => {
      ...
      try {
        ...
        console.log(`[BOSType2Section] Loaded ${preferred.length} preferred equipment for type: "${values.equipmentType}" → "${equipmentType}"`);
        ...
        console.error("[BOSType2Section] Error loading preferred equipment:", error);
      } finally {
  ...
  useEffect(() => {
    ...
    if (prevEquipmentType.current && prevEquipmentType.current !== values.equipmentType && values.equipmentType) {
      ...
      if (currentAmpRating || currentMake || currentModel) {
        console.log(`[BOS Type2] Equipment type changed from "${prevEquipmentType.current}" to "${values.equipmentType}", clearing dependent fields`);
        onChange("ampRating", "");
        onChange("make", "");
        onChange("model", "");
      }
  ...
  useEffect(() => {
    ...
    if (prevAmpRating.current && prevAmpRating.current !== values.ampRating && values.ampRating) {
      ...
      if (currentMake || currentModel) {
        console.log(`[BOS Type2] Amp rating changed from "${prevAmpRating.current}" to "${values.ampRating}", clearing make/model`);
        onChange("make", "");
        onChange("model", "");
      }
    }
  ...
  useEffect(() => {
    ...
    if (prevMake.current && prevMake.current !== values.make && values.make) {
      ...
      if (currentModel) {
        console.log(`[BOS Type2] Make changed from "${prevMake.current}" to "${values.make}", clearing model`);
        onChange("model", "");
      }
    }
  ...
  useEffect(() => {
    ...
    console.log(`[BOS Type2 Auto-select] Auto-selecting amp rating: ${closestAmpString} (min required: ${minRequiredAmps.toFixed(2)})`);
  ...
  useEffect(() => {
    if (!values.ampRating || values.make) {
      console.log(`[BOS Type2 Auto-select] Skipping make auto-select: ampRating=${values.ampRating}, make=${values.make}`);
      return;
    }
    ...
    console.log(`[BOS Type2 Auto-select] Equipment type: "${values.equipmentType}", Standard name: "${standardName}", Amp: ${values.ampRating}, Available makes: ${availableMakes.length}`, availableMakes);

    if (availableMakes.length === 1) {
      console.log(`[BOS Type2 Auto-select] Auto-selecting make: ${availableMakes[0]}`);
      onChange("make", availableMakes[0]);
    } else if (availableMakes.length > 1) {
      console.log(`[BOS Type2 Auto-select] Multiple makes available, user must choose`);
    } else {
      console.log(`[BOS Type2 Auto-select] No makes found for ${standardName} at ${values.ampRating}A`);
    }
  ...
  useEffect(() => {
    ...
    if (availableModels.length === 1) {
      console.log(`[BOSType2Section] Auto-selecting single available model: ${availableModels[0]} (was: ${values.model})`);
      onChange("model", availableModels[0]);
    } else if (availableModels.length > 1) {
      console.log(`[BOSType2Section] Multiple models available (${availableModels.length}), user must select`);
    }
  ...
  console.log(`[BOSType2Section] After amp filtering (${values.ampRating}A): ${allMakesFromCatalog.length} makes available from catalog`);
  console.log(`[BOSType2Section] After preferred filtering: ${makes.length} makes shown in dropdown: ${JSON.stringify(makes)}`);
  ...
  return (
    ...
    >
      ...
      {/* Add BOS Type 3 Button */}
      {showAddBOSType3Button && onAddBOSType3 && (
        <View style={styles.addBOSButtonContainer}>
          <SystemButton
            label={addButtonLabel}
            onPress={onAddBOSType3}
...
export default BOSType2Section;
