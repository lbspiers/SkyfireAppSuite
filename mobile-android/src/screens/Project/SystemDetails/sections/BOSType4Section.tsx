// src/screens/Project/SystemDetails/sections/BOSType4Section.tsx
...
interface BOSType4SectionProps {
  values: {
    isNew: boolean;
...
const BOSType4Section: React.FC<BOSType4SectionProps> = ({
  values,
  onChange,
  errors,
  ...
  useEffect(() => {
    const loadPreferred = async () => {
      ...
      try {
        ...
        console.log(`[BOSType4Section] Loaded ${preferred.length} preferred equipment for type: "${values.equipmentType}" → "${equipmentType}"`);
        ...
        console.error("[BOSType4Section] Error loading preferred equipment:", error);
      } finally {
  ...
  useEffect(() => {
    ...
    if (availableModels.length === 1) {
      console.log(`[BOSType4Section] Auto-selecting single available model: ${availableModels[0]} (was: ${values.model})`);
      onChange("model", availableModels[0]);
    } else if (availableModels.length > 1) {
      console.log(`[BOSType4Section] Multiple models available (${availableModels.length}), user must select`);
    }
  ...
  return (
    ...
    >
      ...
      {/* Add BOS Type 5 Button */}
      {showAddBOSType5Button && onAddBOSType5 && (
        <View style={styles.addBOSButtonContainer}>
          <SystemButton
            label="Add Pre-Combine BOS Equipment"
            onPress={onAddBOSType5}
...
export default BOSType4Section;
